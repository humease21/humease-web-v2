...
export type Product = {
  ...
  review: string;
  faq?: { q: string; a: string; note?: string }[];
...
export const products: Product[] = [
  {
    ...
    reviewChecklist: [
      '데이터 증가량과 보존 기간',
      '검색을 실제로 사용하는 사용자와 권한',
      '기존 Exchange 구조와 가용성 요구',
      '운영 역할 분담',
      '신규 도입인지 기존 환경 이관인지',
    ],
    intro:
    ...
    faq: [
      { q: '보존 정책과 검색은 어떤 관계인가요?',
        a: '보존 정책은 무엇을 얼마나 남길지 정하고, 검색은 남긴 기록을 필요한 순간 찾아내는 역할입니다. 두 요구가 다르면 아카이빙 범위와 인덱싱 구성도 달라지므로 함께 설계합니다.' },
      { q: '사서함 아카이빙과 규정 준수 목적 수집은 같은가요?',
        a: '사서함 아카이빙은 사용자 메일의 보관과 이용을 다루고, 규정 준수 목적 수집은 검토·감독 요구에 맞춘 범위와 절차를 다룹니다. 목적이 다르면 대상 데이터와 보존 기간도 달라집니다.' },
      { q: '기존 환경을 검토할 때 무엇부터 확인하나요?',
        a: '현재 메일 구성, 데이터 규모와 증가 추이, 기존 보존 방식, 운영 역할을 먼저 정리합니다. 신규 도입과 이관은 서로 다른 조건으로 검토합니다.' },
    ],
    related: [
  ...
  },
  {
    ...
    faq: [
      { q: 'Merge1과 Enterprise Vault Capture는 다른 제품인가요?',
        a: '현재 공식 제품 페이지는 Enterprise Vault Capture를 formerly Merge1으로 안내합니다. 휴미즈에서도 두 이름을 별도 제품으로 나누지 않고 같은 제품의 현재 이름과 이전 이름으로 설명합니다. 기존 계약의 제품 구성과 권리는 별도로 확인해야 합니다.' },
      { q: '어떤 데이터를 수집하나요?',
        a: 'Microsoft Teams, Slack 등 협업·커뮤니케이션 데이터 수집을 지원합니다. 실제 수집 항목과 방식은 커넥터, 원천 서비스 권한과 구성에 따라 확인해야 합니다.' },
      { q: '수집과 장기 보존은 같은 역할인가요?',
        a: '수집은 원천 데이터와 관련 정보를 가져오는 단계이고, 장기 보존은 정해진 정책으로 보관·관리하는 단계입니다. Capture의 수집 범위와 연결되는 아카이브의 보존 정책을 구분해 검토해야 합니다.' },
      { q: '기존 Merge1 환경을 검토하려면 무엇이 필요한가요?',
        a: '사용 버전, 수집 대상, 연결된 아카이브, 라이선스와 현재 운영 조건을 먼저 정리합니다. 이름 변경만으로 기존 환경의 지원 여부나 무상 업그레이드를 단정하지 않고 해당 계약·기술 자료를 확인합니다.',
        note: '이 항목은 휴미즈의 검토 접근 방식이며, 제조사가 보장하는 계약 조건이 아닙니다.' },
    ],
    related: [
  ...
  },
  {
    ...
    faq: [
      ...
      { q: '분석 대상은 무엇인가요?',
        a: '지원되는 저장소의 파일 현황, 사용·접근 활동, 확인 가능한 소유·관리 단서를 분석합니다. 실제 대상 범위는 구성에 따라 확인해야 합니다.' },
      { q: '소유권·접근 정보는 어떻게 활용하나요?',
        a: '관리 공백이 있는 영역을 파악하고 정리·보존 판단의 우선순위를 정하는 데 사용합니다. 분석 결과를 실제 삭제나 권한 변경으로 연결할 때는 별도 승인·검증 절차를 둡니다.' },
      { q: '아카이브·백업과 어떻게 다른가요?',
        a: '아카이브와 백업은 데이터를 보관하는 역할이고, Data Insight는 데이터의 현황과 사용을 분석해 판단을 돕는 역할입니다. 보관 자체를 대신하지 않습니다.' },
    ],
    related: [
  ...
  },
  {
    ...
    faq: [
      ...
      { q: 'Legal Hold·수집·검토는 각각 어떤 역할인가요?',
        a: 'Legal Hold는 조사에 필요한 기록의 삭제를 제한하고, 수집은 관련 자료를 모으며, 검토는 자료를 분류·태깅해 필요한 내용을 확인하는 단계입니다.' },
      { q: '아카이브와는 어떻게 연결되나요?',
        a: '아카이브에 보존된 기록이 조사 대상이 될 수 있습니다. 연결 방식과 접근 권한은 제품 구성과 환경에 따라 확인해야 합니다.' },
      { q: '자료 제출 전에 무엇을 확인해야 하나요?',
        a: '제출 형식, 마스킹 범위, 처리 이력의 기록 여부를 확인합니다. 법률적 판단과 제출 적합성의 최종 확인은 고객의 법무·컴플라이언스 담당 범위입니다.' },
    ],
    related: [
      { label: 'e-Discovery 컨설팅', href: '/consulting/e-discovery' },
      { label: 'Enterprise Vault Complete', href: '/solutions/enterprise-vault' },
