...
- 개정 2026-09-06 (v2) — 현행 구현 기준. v1 의 Vercel·서버 API 전제는 폐기됐다(`ADR-001`, `SPEC.md` §9).

- G0 조사는 완료됐다. 결과는 `docs/reports/G0_BASELINE.md` 에 있다.
...
새 저장소 이름은 `humease-web-v2`로 정한다. **G0 확인 결과** 실제 저장소는 `humease21/humease-web-v2`(public)이며, 두 로그인 계정 모두 구 저장소 `Humease/Homepage` 에 `pull` 권한만 있다. 동일 이름의 저장소·폴더가 발견되면 소유와 내용을 확인하고 덮어쓰기·삭제·강제 초기화하지 않는다.

## 2. 현재 정보의 신뢰 수준

| 분류 | 내용 | 처리 |
|---|---|---|
| 확정 | Next.js 신규 구축, **GitHub Pages 배포**, 이미지 생성 완료 | 설계 기준으로 적용 |
| 사용자 제공 주소 | `https://www.humease.com` | 최종 대표 도메인 기준 |
| G0에서 확인 완료 | 운영 커밋 `afaa576`, DNS, 이미지 매핑, 문의 기능 계약 | `docs/reports/G0_BASELINE.md` |
| 여전히 미확인 | DB·RLS·인증 상세, 정책 승인본, 회사 공개 정보 | BLOCKED. 추정하지 않음 |

구 운영 저장소는 G0 에서 READ-ONLY 로 실제 조회했다. 결과는 `docs/reports/G0_BASELINE.md` 에 있다.
조사하지 않은 항목을 확정된 사실로 승계하지 않는다.
...
| 새 저장소 | Next.js 초기화, 작업 브랜치, 승인 후 merge | 구 저장소를 remote origin 으로 연결 |
| 신규 GitHub Pages | 검증 배포와 색인 차단 상태 유지 | 운영 도메인 무단 연결, 구 Pages 종료 |
...
7. 배포 환경(GitHub Pages)·색인 정책·환경변수 준비 여부.
